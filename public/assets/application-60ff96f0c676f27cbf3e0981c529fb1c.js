...
(function() {
  ...
  controllers.controller("PhotoListCtrl", [
    "$scope", "$http", "$q", "$timeout", "$location", "app", function($scope, $http, $q, $timeout, $location, app) {
      var map, photos;
      map = null;
      photos = app;
      $scope.distanceArray = [0.3, 0.5, 1.0, 1.5, 2.0, 3.0, 5.0, 7.0, 10.0, 15.0, 20.0, 30.0, 40.0, 50.0];
      $scope.createMap = function(_arg) {
        var latitude, longitude, mapDiv, mapOptions, marker, myLatlng;
        latitude = _arg.latitude, longitude = _arg.longitude, mapDiv = _arg.mapDiv;
      ...
      $scope.morePhotos = function() {
        return $scope.photosMax += 32;
      };
      $scope.restartMaxPhotos = function() {
        return $scope.photosMax = 32;
      };
      $scope.heightMap = {
        "height": '100px'
      ...
      $scope.changeViewOrganization = function(_arg) {
        var type;
        type = _arg.type;
        if (type === "grid") {
          $scope.listDisplay = {
      ...
      $scope.myFilter = function(o) {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          keyword = _ref[_i];
          if (o.tags.search(keyword) === -1 || o.distance > $scope.distanceArray[$scope.distanceArrayIndex]) {
            return false;
          }
        }
        if (($scope.foodType !== void 0 && o.tags.search($scope.foodType.tags) === -1) || o.distance > $scope.distanceArray[$scope.distanceArrayIndex]) {
          return false;
        }
        return true;
      };
      $scope.pushKeyWord = function(value) {
        $scope.keywords.push(value);
        return $scope.foodType.tags = "";
      };
      $scope.test = function(_arg) {
        var value;
        value = _arg.value;
        return console.log(value);
      };
      $scope.setHeightMap = function(_arg) {
        var value;
        value = _arg.value;
        $scope.heightMap = {
          "height": value
        ...
        return $timeout((function() {
          return $scope.setCurrentPhoto($scope.currentPhoto);
        }), 50);
      };
      $scope.increaseDistance = function() {
        if ($scope.distanceArrayIndex < $scope.distanceArray.length - 1) {
          return $scope.distanceArrayIndex += 1;
        }
      };
      return $scope.decreaseDistance = function() {
...
(function() {
  ...
  directives.directive('keyword', function() {
    return {
      restrict: "E",
      transclude: true,
  ...
  directives.directive('showmap', function() {
    return {
      restrict: "E",
      scope: {
        createMap: "&",
      ...
      template: '<div><a href="" ng-click="showMap()" ' + 'ng-show="showMapLinkVisibility">View on the map</a>' + '<a href="" ng-click="showMap()" ng-show="!showMapLinkVisibility">' + 'Hide map</a></div>' + '<div class="mapcanvas" id="mapdiv{{photo.instagram_url}}" ng-show="!showMapLinkVisibility"></div>',
      controller: function($scope, $timeout) {
        $scope.showMapLinkVisibility = true;
        return $scope.showMap = function() {
          console.log(!$scope.showMapLinkVisibility);
          $scope.showMapLinkVisibility = !$scope.showMapLinkVisibility;
          return $timeout((function() {
            return $scope.createMap({
              latitude: $scope.photo.latitude,
              longitude: $scope.photo.longitude,
              mapDiv: "mapdiv" + $scope.photo.instagram_url
            });
          }), 10);
        };
      }
  ...
  directives.directive('buttonpannel', function() {
    return {
      restrict: "E",
      scope: {
        glyphclass1: "@",
      ...
      controller: function($scope) {
        ...
        $scope.go1 = function() {
          ...
          return $scope.actionfunction1();
        };
        return $scope.go2 = function() {
          $scope.visibility1 = !$scope.visibility1;
          return $scope.actionfunction2();
        };
      },
      template: '<button class="btn btn-default btn-lg" type="submit" ' + 'ng-click="go1()" ng-show="visibility1">' + '<span class="glyphicon {{glyphclass1}}"></span></button>' + '<button class="btn btn-default btn-lg" type="submit" ' + 'ng-click="go2()" ng-hide="visibility1">' + '<span class="glyphicon {{glyphclass2}}"></span></button>'
    };
  });
      ...
      resolve: {
        app: function($q, $http) {
          ...
          navigator.geolocation.getCurrentPosition(function(position) {
            ...
            return $http.get("/photos?latitude=" + location.latitude + "&longitude=" + location.longitude).success(function(data) {
              photos = data;
