...
(function() {
  ...
  app.controller("PhotoListCtrl", [
    "$scope", "$http", "$q", "$timeout", function($scope, $http, $q, $timeout) {
      var defer, latitude_user, longitude_user, map;
      map = null;
      latitude_user = 0;
      longitude_user = 0;
      defer = $q.defer();
      defer.promise.then(function() {
        return $http.get("/photos?latitude=" + latitude_user + "&longitude=" + longitude_user).success(function(data) {
          $scope.photos = data;
          $scope.setCurrentPhoto(data[0]);
          return $scope.tags = JSON.parse(data[0].tags);
        });
      });
