...
(function() {
  ...
  app.controller("PhotoListCtrl", [
    "$scope", "$http", "$q", function($scope, $http, $q) {
      var defer, latitude_user, longitude_user;
      latitude_user = 0;
      longitude_user = 0;
      defer = $q.defer();
      defer.promise.then(function() {
        return $http.get("/photos?latitude=" + latitude_user + "&longitude=" + longitude_user).success(function(data) {
          ...
          return $scope.tags = JSON.parse($scope.currentPhoto.tags);
        });
      });
      ...
      $scope.setCurrentPhoto = function(photo) {
        ...
        return google.maps.event.addDomListener(window, "load", initialize_map());
      };
      $scope.photosMax = 32;
      $scope.morePhotos = function() {
        return $scope.photosMax += 32;
      };
      return $scope.restartMaxPhotos = function() {
        return $scope.photosMax = 32;
      };
    }
  ]);
